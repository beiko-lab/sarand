...
def haveContent(config_file):
	"""
	To check if the config file has any parameter set or everything is commented out.
	"""
	file1 = open(config_file, 'r')
	lines =file1.readlines()
	for line in lines:
		if len(line.strip())>1 and line.strip()[0]!='#' and line[0:3]!='---':
			return True
	return False
...
def update_full_pipeline_params(args, params):
	...
	if args.main_dir is not None:
		arg_main_dir = args.main_dir
	if os.path.realpath(arg_main_dir)!=os.path.realpath(params.main_dir):
			#changes params variables dependant on main_dir only accessible through params.py
			params.output_dir = params.output_dir.replace(params.main_dir.rstrip(' /'), arg_main_dir.rstrip(' /'))
			params.amr_files = params.amr_files.replace(params.main_dir.rstrip(' /'), arg_main_dir.rstrip(' /'))
			params.ng_seq_files = params.ng_seq_files.replace(params.main_dir.rstrip(' /'), arg_main_dir.rstrip(' /'))
			params.ng_path_info_files = params.ng_path_info_files.replace(params.main_dir.rstrip(' /'), arg_main_dir.rstrip(' /'))
			params.main_dir = arg_main_dir
...
def update_full_pipeline_params(args, config_data, params):
	"""
	"""
	config_params = config_data.keys()
	user_main_dir =''
	if args.main_dir is not None:
	...
	if os.path.realpath(user_main_dir)!=os.path.realpath(params.main_dir):
			...
			params.amr_files = params.amr_files.replace(params.main_dir.rstrip(' /'), user_main_dir.rstrip(' /'))
			params.ng_seq_files = params.ng_seq_files.replace(params.main_dir.rstrip(' /'), user_main_dir.rstrip(' /'))
			params.ng_path_info_files = params.ng_path_info_files.replace(params.main_dir.rstrip(' /'), user_main_dir.rstrip(' /'))
			params.main_dir = user_main_dir
